...
import (
	...
	caretta_k8s "github.com/gelonsoft/caretta/pkg/k8s"
	"github.com/gelonsoft/caretta/pkg/metrics"
...
var (
	linksMetrics = promauto.NewGaugeVec(prometheus.GaugeOpts{
		...
		"client_ip", "server_ip", "link_type", "client_port",
	})
)
...
func (caretta *Caretta) handleLink(link *NetworkLink, throughput uint64) {
	linksMetrics.With(prometheus.Labels{
		...
		"client_port":      strconv.Itoa(int(link.ClientPort)),
